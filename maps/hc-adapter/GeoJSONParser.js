...
anychart.core.utils.GeoJSONParser.prototype.convert = function(data, txObj) {
  var i, len;
  this.txObj_ = txObj;
  ...
  switch (data['type']) {
    case 'FeatureCollection':
      if (!data['features']) {
        ...
        for (i = 0, len = features.length; i < len; i++) {
          ...
          // this.transformProp_(feature['properties']);
        }
      }
      break;
    ...
    case 'Feature':
      if (!(data['properties'] && data['geometry'])) {
        ...
        // this.transformProp_(data['properties']);
      }
      break;
  ...
  data['ac-tx'] = {
    'default': {'crs': this.txObj_.default.crs}
  };
 ...
 * @return {Object}
 */
anychart.core.utils.GeoJSONParser.prototype.convert_ = function(data, txObj, donor, acFieldName, hcFieldName) {
  var i, len;
  this.txObj_ = txObj;
...
anychart.core.utils.GeoJSONParser.prototype.transform_ = function(coord) {
  ...
  var tx = this.txObj_.default;

  var normalized = {
    x: ((x - (tx.jsonmarginX || 0)) / (tx.jsonres || 1) - (tx.xpan || 0)) / (tx.scale || 1) + (tx.xoffset || 0),
    y: ((y - (tx.jsonmarginY || 0)) / (tx.jsonres || 1) + (tx.ypan || 0)) / (tx.scale || 1) + (tx.yoffset || 0)
  };

  var cosAngle = tx.cosAngle || (tx.rotation && Math.cos(tx.rotation));
  var sinAngle = tx.sinAngle || (tx.rotation && Math.sin(tx.rotation));

  normalized = tx.rotation ?
  {
